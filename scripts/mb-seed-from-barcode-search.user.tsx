...
function main() {
    ...
    let focused = false
    for (const td of document.querySelectorAll<HTMLTableCellElement>("td.barcode-cell")) {
        if (removeFirstZeroes(barcode) === removeFirstZeroes(td.textContent.trim())) {
            td.parentElement!.style.border = "2px solid blue"
            const link = td.parentElement!.querySelector<HTMLAnchorElement>('a[href*="release/"]:not([href*=cover-art])')
            if (link == null) continue
            let focusTarget: HTMLElement = link
            if (seedUrl) {
                const form = document.createElement("form")
                form.method = "POST"
