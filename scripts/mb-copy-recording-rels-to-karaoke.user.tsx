...
async function doIt(button: HTMLButtonElement, doReverse: boolean) {
    const MB = window.MB
    ...
    const dstRecordingStates = new Map<string, MediumRecordingStateT>()
    ...
    const recToRecRelationships = new Map<`${string}:${string}`, {
        srcRecordingId: string,
        dstRecordingId: string,
        karaoke: boolean,
        partial: boolean,
    }>()
    for (const dstRecording of Array.from(/* currentRecordings はループ内で変化するので Array.from で確定させる */currentRecordings.values())) {
        ...
        for (const srcRecordingRel of srcRecordingRels) {
            ...
            if (srcRecording.entityType !== "recording") continue // ?
            ...
            if (obj == null) {
                ...
                if (!currentRecordings.has(srcRecording.gid)) currentRecordings.set(srcRecording.gid, srcRecording)
    ...
    await resolveRecordingsRelationships(
        Array.from(recToRecRelationships.values())
    ...
    for (const recToRecRel of recToRecRelationships.values()) {
        ...
        for (const rel of srcRecording.relationships!) {
            ...
            if (rel.linkTypeID === WORK_REL_LINK_TYPE_ID) {
                if (doReverse) {
                    const removeAttrIds = new Set<number>()
                    if (recToRecRel.karaoke) removeAttrIds.add(WORK_REL_KARAOKE_LINK_ATTR_TYPE_ID)
                    if (recToRecRel.partial) removeAttrIds.add(WORK_REL_PARTIAL_LINK_ATTR_TYPE_ID)
                    attrs = attrs.filter(x => !removeAttrIds.has(x.typeID))
                } else {
                    const neededAttrIds = new Set<number>()
                    if (recToRecRel.karaoke) neededAttrIds.add(WORK_REL_KARAOKE_LINK_ATTR_TYPE_ID)
                    if (recToRecRel.partial) neededAttrIds.add(WORK_REL_PARTIAL_LINK_ATTR_TYPE_ID)
                    
                    for (const neededAttrId of neededAttrIds) {
                        if (attrs.find(x => x.typeID === neededAttrId) != null) continue
                ...
                const dstRecWorkRel = dstRecWorkRels?.find(r => 
                    rel.backward
            ...
            editor.dispatch({
                ...
                newRelationshipState: {
                    ...
                    _status: oldRelationshipState ? (oldRelationshipState._status || 2 satisfies REL_STATUS_EDIT_T) : (1 satisfies REL_STATUS_ADD_T),
...
const elm = <div>
    ...
    }}>Copy Recording Relationships to Karaoke/Edited Recordings</button>
    <button onClick={e => {
        const button = e.currentTarget
        button.disabled = true
    ...
    }} style={{marginLeft: "1em"}}>... or FROM Karaoke/Edited Recordings</button>
</div>
