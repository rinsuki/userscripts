import { ReleaseT } from "typedbrainz/types"
...
function main() {
    const table = document.querySelector<HTMLTableElement>("table.mergeable-table")
    if (table == null) return
    for (const row of table.rows) {
        const link = row.querySelector<HTMLAnchorElement>("td > a[href^='/release/'][href*='-']:has(bdi)")
        ...
        button.addEventListener("click", e => {
            ...
            fetch(`/ws/js/release/${new URL(link.href).pathname.split("/")[2]}`).then(r => r.json()).then((r: ReleaseT) => {
                let f: Record<string, string | undefined | null> = {
                    name: r.name,
